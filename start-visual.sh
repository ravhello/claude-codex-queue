...
if [ -n "${CLAUDE_VSCODE_QUEUE_STATE:-}" ]; then
  ...
  if [ -n "$WIN_HOME_RAW" ]; then
    STATE="$(wslpath -u "$WIN_HOME_RAW")/.claude-vscode-queue"
  else
    STATE="$HOME/.claude-vscode-queue"
  fi
else
  STATE="$HOME/.claude-vscode-queue"
fi
...
stop_stale() {
  ...
  if [[ "$args" == *"claude_vscode_queue.web --host $HOST --port $PORT"* ]]; then
...
stop_stale
setsid -f python3 -m claude_vscode_queue.web --host "$HOST" --port "$PORT" > "$LOG" 2>&1 < /dev/null

for _ in $(seq 1 30); do
  if is_alive; then
    pgrep -f "[c]laude_vscode_queue.web --host $HOST --port $PORT" | tail -1 > "$PID_FILE" || true
    echo "http://$HOST:$PORT/"
    exit 0
  fi
  sleep 0.5
